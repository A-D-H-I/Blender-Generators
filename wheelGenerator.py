...
class GenerateWheelOperator(bpy.types.Operator):
    ...
    def create_spokes(self, rim_radius, tire_thickness, count):
        spokes = []
        # The tire is created as a torus with a fixed minor_radius=0.05.
        # Its inner boundary is roughly calculated as:
        inner_tire_radius = rim_radius + tire_thickness - 0.05
        # Spoke length: gap from rim outer border to tire inner border.
        ...
        for i in range(count):
            ...
            # Rotate the cylinder: first rotate 90° about Y to lay it horizontally,
            # then rotate around Z so it points radially outward.
            spoke.rotation_euler[1] = math.radians(90)
            spoke.rotation_euler[2] = angle

            # Position it so its inner end touches the rim.
    ...
    def create_tire_material(self):
        ...
        if not mat:
            ...
            # Create nodes for tire material
            ...
            # Create a Bump node to add surface detail
            bump = nodes.new(type='ShaderNodeBump')
            bump.location = (-200, 0)
            bump.inputs["Strength"].default_value = 0.3

            # Link the Noise Texture to the Bump node's Height input
            links.new(noise.outputs["Fac"], bump.inputs["Height"])
            # Link the Bump node's Normal output to the BSDF's Normal input
            links.new(bump.outputs["Normal"], bsdf.inputs["Normal"])
            # Finally, link the BSDF output to the Material Output
